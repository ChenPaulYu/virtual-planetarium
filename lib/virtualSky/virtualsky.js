...
VirtualSky.prototype.drawConstellationLines = function(colour){
	...
	for(var c = 0; c < this.lines.length; c++){
		...
		if (c == ref && ref == use[index].number) {
			...
			// base_image.src = 'http://st2.ilvs.ilc.edu.tw/~s414012/images/%E5%9C%96%E6%96%87.png'
			base_image.src = './picture/Cartoon.png'
			
			// base_image.src = "./picture/" + use[index].picture;			
			base_image.onload = function () {
				// console.log(cx, cy)
				x.globalAlpha = 0.4;
				if (f < 40 && f > 30 && cx_array[0] != -1 && cy_array[0] != -1) {
					x.drawImage(base_image, cx_array[0] + use[index].offsetx, cy_array[0] + use[index].offsety, use[index].height * (50 / f), use[index].height * (50 / f));
				}
				x.resetTransform();
			}
		...
		if(this.constellation.labels){
			...
			// if (pos.x > -1 && pos.y > -1 && this.fov < 40 && label == "Perseus") {
			// 	console.log(pos.x, pos.y, label)
			// 	base_image = new Image();
			// 	base_image.src = 'http://st2.ilvs.ilc.edu.tw/~s414012/images/圖文.png'
			// 	base_image.onload = function () {
			// 		x.globalAlpha = 0.4;
			// 		x.drawImage(base_image, pos.x + xoff, pos.y - fontsize / 2);
			// 		x.resetTransform();
			// 	}
