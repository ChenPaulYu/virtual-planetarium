...
function VirtualSky(input){
	...
	this.file = {
		...
		use: this.dir+"constellation.json"
	}
...
VirtualSky.prototype.init = function(d){
	...
	if (is(d.target, s)) this.target = d.target;
	if(is(d.callback,o)){
...
VirtualSky.prototype.setFOV = function(fov){
	if(fov >= 60 || typeof fov!=="number") {
		...
		this.fov = 35
		// this.toggleInfoBox(this.target, true)
		setTimeout(() => {
			this.toggleInfoBox(this.target, true)
		}, 500);
	}else {
...
VirtualSky.prototype.checkLoaded = function(){
	...
	if(!this.use) this.load('use', this.file.use);
...
VirtualSky.prototype.createSky = function(){
	...
	setTimeout(() => {
		...
		for(var i=0;i<this.use.length;i++) {
			var temp = loader.addImage("./image/constellations/" + this.use[i].picture);
			this.image.push(temp)
		}

		loader.addCompletionListener(function () {
			console.log('complete')
			console.log(this.use[10].offsetx)
			console.log(this.use[11].offsetx)
		});
	...
	if(this.objects){
		...
		// If you need more objects (e.g. for the x catalogue) build a JSON
...
VirtualSky.prototype.guide  = function () {
	// this.container.append(`<div id="guide" class="guide">
	//     <div id="guide_inside">
	//         <img id="move" src="./image/guide/KwL8Vdb.png" />
	//     </div>
	// </div>`)
	this.container.append(`
		<img class='guideAll' id="gestureguide" src="" />
		<img class='guideAll' id="move" src="" />
	`)
	

}

VirtualSky.prototype.Anime = function (i) {
	Anime(i);
}
VirtualSky.prototype.restart = function () {
	setState(0);
	Anime(0);
	setState(1);
	setTimeout(() => {
...
VirtualSky.prototype.toggleInfoBox = function(i,show){
	...
	// 	i = this.target
	// }
	// if(this.fov < 40 && this.fov >= 30) {
	// 	console.log('display')
	// 	i = this.target
	// 	var offset = this.container.position();
	// 	el.html(this.pointers[i].html);
	// 	var x = Math.round(this.pointers[i].x - 2 * Math.round(el.outerWidth() / 2)) + 'px';
	// 	var y = Math.round(this.pointers[i].y - 0 * Math.round(el.outerHeight() / 2)) + 'px';
	// 	el.css({'position':'absolute','left':x,'top':y,'z-index':10,'display':'block','padding':'0px','border-radius':'5px'});
	// }else {
	// 	console.log('none')
	// 	el.css({
	// 		'display': 'none'
	...
	// if(i >= 0 && this.isVisible(this.pointers[i].el) && this.pointers[i].x > 0 && this.pointers[i].y > 0 && this.pointers[i].x < this.wide && this.pointers[i].y < this.tall){
		var scaleX = this.pointers[i].scaleX
		var scaleY = this.pointers[i].scaleY
		el.html(this.pointers[i].html);
		var x = Math.round(this.pointers[i].x - scaleX * this.pointers[i].width) + 'px';
		var y = Math.round(this.pointers[i].y - scaleY * this.pointers[i].height) + 'px';
		// console.log(`x:${x},y:${y}`)
		el.css({'position':'absolute','left':x,'top':y,'z-index':10,'display':'block','padding':'0px','border-radius':'5px','opacity':'0'})
	...
	for(i = 0; i < this.stars.length; i++){
		if(this.stars[i][1] < this.magnitude){
			...
			if(this.isVisible(p.el) && !isNaN(p.x) && !this.isPointBad(p)){
				...
				c.moveTo(p.x+d,p.y);
				// c.globalAlpha = 0.3
				if(this.showstars) c.arc(p.x,p.y,d*1.1,0,Math.PI*2,true);
...
VirtualSky.prototype.drawConstellationLines = function(colour){
	...
	for(var c = 0; c < this.lines.length; c++){
		...
		if(this.use) {
			for(var a = 0;a < this.use.length ;a++) {
				if(this.use[a].number == c) {
					ref = c;
				}
			}
		}
		...
		// Image Note
		if (c == ref && ref == use[index].number) {
			cx += lastx
			cy += lasty 
			cx /= (count+1)
			cy /= (count+1)
			
			if (f == 35 && cx != -1 && cy != -1 && !this.animate) {
				x.globalAlpha = this.alpha
				if(this.alpha < 1) {
					this.alpha += 0.01
				}
				x.drawImage(this.image[index], cx + use[index].offsetx, cy + use[index].offsety, use[index].width * (1), use[index].height * (1));
			}

			x.globalAlpha = 1
			
			// base_image = new Image();
			// base_image.src = "https://i.imgur.com/" + use[index].picture;

			// base_image.onload = function () {
			// 	x.globalAlpha = 1;
			// 	if (f == 35 && cx_array[0] != -1 && cy_array[0] != -1) {
			// 		x.drawImage(base_image, cx_array[0] + use[index].offsetx, cy_array[0] + use[index].offsety, use[index].width * (1), use[index].height * (1));
			// 	}
			// 	x.resetTransform();
			// }
		...
		if(this.constellation.labels){
			...
			if(this.isVisible(pos.el)){
				...
				x.fillText(label,pos.x,pos.y-fontsize/2)
				x.fill();
			}
		}
	}
...
VirtualSky.prototype.addPointer = function(input){
	...
	if(matched < 0){
		...
		if (typeof p.html !== "string") {
			style = p.style || `width:${p.width}px; height:${p.height}px;margin:0px;border-radius:5px;`;
