...
VirtualSky.prototype.drawConstellationLines = function(colour){
	...
	for(var c = 0; c < this.lines.length; c++){
		console.log(this.getPhrase('constellations', this.lines[7][0]))
		var cx = 0
		var cy = 0
		var lastx = 0
		var lasty = 0
		...
		if (c == 7) {
			...
			base_image.onload = function () {
				...
				if (f < 40 && f > 30 && cx_array[0] != -1 && cy_array[0] != -1) {
					x.drawImage(base_image, cx_array[0] - 100, cy_array[0] - 100, base_image.width * (50 / f), base_image.height * (50 / f));
				}
				x.resetTransform();
			}
